# ============================================
# K-MEANS CLUSTERING - Dry and Wet Seasons
# ============================================
...
create_kmeans_plot <- function(data, sites, season_name) {
  ...
  ggplot() +
    geom_point(data = scores, aes(x = PC1, y = PC2, color = Cluster), size = 4, alpha = 0.8) +
    stat_ellipse(data = scores, aes(x = PC1, y = PC2, color = Cluster), linetype = "dashed", level = 0.95) +
    geom_text(data = scores, aes(x = PC1, y = PC2, label = Site), size = 4, fontface = "bold", vjust = -1, color = "black") +
    ...
    scale_color_manual(values = c("#D32F2F", "#388E3C", "#1976D2")) +
    theme_classic() +
    ...
    labs(title = paste(season_name, "- K-means Clustering"),
...
dry_kmeans <- create_kmeans_plot(dry_data, dry_site, "Dry Season")
wet_kmeans <- create_kmeans_plot(wet_data, wet_site, "Wet Season")

print(dry_kmeans)
print(wet_kmeans)

ggsave("Dry_Season_Kmeans.tiff", dry_kmeans, width = 11, height = 9, dpi = 600)
ggsave("Wet_Season_Kmeans.tiff", wet_kmeans, width = 11, height = 9, dpi = 600)
